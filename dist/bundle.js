...
/***/ function(module, exports, __webpack_require__) {
	...
	var loginView = __webpack_require__(11);

	var checkLogin = function checkLogin() {
	  var auth = user.isLoggedIn();
...
/***/ function(module, exports, __webpack_require__) {
	...
	var fbRef = new Firebase('https://ssgestimator.firebaseio.com/');
	var fbProjects = fbRef.child('projects');
	var fbUsers = fbRef.child('users');
	var fbAdmins = fbRef.child('admins');
	var project = __webpack_require__(4);
	var indexView = __webpack_require__(5);
	var userController = __webpack_require__(10);

	var create = function create(email, pwd) {
	  fbRef.createUser({
	    email: email,
	    password: pwd
	...
	var authenticate = function authenticate(pwd) {
	  fbRef.authWithPassword({
	    ...
	    if (error) {
	      ...
	      isAdmin(exports.uid).then(function (admin) {
	        if (admin) {
	          getAllProjects();
	        } else {
	...
	var isAdmin = function isAdmin(uid) {
	  return fbAdmins.child(uid).once('value').then(function (s) {
	    return s.val();
	  });
	};

	var setProjectOwner = function setProjectOwner(newProject) {
	  var userRef = fbUsers.child(exports.uid);
	  var obj = {};
	...
	var getProjectList = function getProjectList(uid) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var $ = __webpack_require__(1);
	var user = __webpack_require__(2);
	var fbProjects = user.fbProjects;

	var projectMaker = function projectMaker(client, city, labor, mkup, owner) {
	  this.client = client;
	  this.city = city;
	...
	var exists = function exists(newProject) {
	...
	var saveNew = function saveNew(newProject) {
	  var obj = {};
	...
	var updateComponent = function updateComponent(cur, key) {
	...
	var populate = function populate(cur) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var $ = __webpack_require__(1);
	var project = __webpack_require__(4);
	var projectController = __webpack_require__(6);
	...
	var handleCreateButton = function handleCreateButton() {
	...
	var handleProjectForm = function handleProjectForm() {
	...
	var handleProjectCancel = function handleProjectCancel() {
	...
	var populateSelector = function populateSelector(project) {
	...
	var handleSelector = function handleSelector() {
	...
	var makeTable = function makeTable(cur) {
	...
	var clearDisplays = function clearDisplays() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var round = function round(op, num, inc) {
	  var x = 1 / inc;
	  return Math[op](num * x) / x;
	};

	var laborCost = function laborCost(hr) {
	  return hr * project.current.laborRate;
	};

	//ADD THE MARKUP TO THIS FUNCTION!
	var materialCost = function materialCost(qty, rate) {
	  return round('round', qty * rate, 0.01);
	};

	var salesTax = function salesTax(price) {
	  return round('round', price * 0.095, 0.01);
	};

	var findObjInArray = function findObjInArray(id, arr, selector) {
	...
	var sumObject = function sumObject(obj) {
	...
	var plucky = function plucky(key, obj) {
	  var sum = 0;
	  for (var prop in obj) {
	...
	var objectStripper = function objectStripper(arr, props) {
	  var newarr = [];
	...
	var sumStrippedProps = function sumStrippedProps(arr, props) {
	  return props.map(function (prop) {

	    return objectStripper(arr, prop).reduce(function (sum, obj) {
	      return sum + obj[prop];
	    }, 0);
	  });
	};
	...
	var picker = function picker(arr, prop) {
	...
	var camelCaseToLowerCase = function camelCaseToLowerCase(str) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var userInit = function userInit() {
	  indexView.init();
