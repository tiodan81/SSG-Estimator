...
cistern.buildCistern = function () {
  ...
  var $pump = $('#cistern-pump:checked').length ? 1 : 0;
  var $div = $('#cistern-diverter:checked').length ? 1 : 0;
  var $gauge = $('#cistern-gauge:checked').length ? 1 : 0;
...
cistern.calculateBaseMaterials = function (c) {
  var modelInfo = cistern.tankModels[c.model];
...
cistern.preventDuplicates = function () {
  var $id = $('#cisternID').val();
  var $exists = util.findObjInArray($id, project.current.cisterns.all, 'id').length;
  if ($exists) {
    return true;
  } else {
    return false;
...
cistern.storeLocally = function (newCistern) {
  var cur = project.current.cisterns.all;
  var $exists = util.findObjInArray(newCistern.id, cur, 'id');
...
cistern.updateUberTank = function () {
  var cisterns = project.current.cisterns;
  var uber = cistern.makeUberTank(cisterns.all);
  cisterns.uber = uber;
...
cistern.makeUberTank = function (arr) {
  var obj = new cistern.cisternMaker();
