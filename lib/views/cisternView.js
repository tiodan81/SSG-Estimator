...
cisternView.init = function () {
  $('#cistern-content').show().siblings().hide();
...
cisternView.displayExisting = function () {
  var $existing = project.current.cisterns;
...
cisternView.handleSave = function () {
  $('#cisternForm').off('submit').on('submit', function (e) {
    e.preventDefault();
    var $val = $('#cistern-save').val();
    if ($val === 'save') {
      var dupe = cistern.preventDuplicates();
      if (dupe) {
...
cisternView.showSummary = function () {
  var $selected = $('#cistern-nav .button-primary').attr('id');
  if ($selected != 'summary') {
    $('#cistern-nav > #summary').addClass('button-primary').siblings().removeClass('button-primary');
    $('#cistern-table-summary').show().siblings().hide();
  }
};

cisternView.handleSelector = function () {
  $('#cistern-selector').off('change').on('change', function () {
    var id = $('#cistern-selector').val();
    if (id === 'All tanks') {
      cisternView.makeTables(project.current.cisterns.uber);
      $('#cistern-edit-buttons').hide();
    } else {
      var curCistern = util.findObjInArray(id, project.current.cisterns.all, 'id');
...
cisternView.handleDelete = function () {
  $('#cistern-edit-buttons .icon-bin2').off('click').on('click', function (e) {
    var old = cistern.current;
    var all = project.current.cisterns.all;
